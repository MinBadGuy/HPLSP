...
public:
    ...
    tw_timer* add_timer(int timeout)
    {
        ...
        /* 下面根据待插入定时器的超时值计算它将在时间轮转动多少个滴答后被触发，并将该滴答数字存储于变量ticks中。
            如果待插入定时器的超时值小于时间纶的槽间隔SI，则将ticks向上这和为1，否则就将ticks向下折合为timeout/SI
        */
        if (timeout < SI)
        {
            ticks = 1;
...
private:
    ...
    static const int SI = 1;    // 每1s时间轮转动一次，即槽间隔1s
