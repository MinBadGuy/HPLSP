...
public:
    ...
    void add_timer(util_timer* timer)
    {
        ...
        /* 如果目标定时器的超时时间小于当前链表中所有定时器的超时时间，则把该定时器插入链表头部，作为链表新的头节点。
            否则就需要调用重载函数 add_timer(util_timer* timer, util_timer* lst_head)，把它插入链表中合适的位置，以保证链表的升序特性
         */
    ...
    */
    void adjust_timer(util_timer* timer)
    {
        ...
        /* 如果被调整的目标定时器处在链表尾部，或者该定时器新的超时时间仍然小于下一个定时器的超时值，则不用调整 */
