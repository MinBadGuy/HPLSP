 ...
 * @param epollfd: 内核事件表
 * @param fd: 文件描述符
 * @param oneshot: 指定是否注册fd上的EPOLLONESHOT事件
 * @return: 
*/
...
/**
 * @brief: 重置fd上的事件。这样操作后，尽管fd上的EPOLLONESHOT事件被注册，但是操作系统仍然会触发fd上的EPOLLIN事件，且只触发一次
 * @param epollfd: 内核事件表
 * @param fd: 文件描述符
*/
...
int main(int argc, char* argv[])
{
    ...
    assert(epollfd != -1);
    /*监听socket listenfd上是不能注册EPOLLONESHOT事件的，否则应用程序只能处理一个客户连接，
    ...
    while (1)
    {
        ...
        for (int i = 0; i < ret; i++)
        {
            ...
            else if (events[i].events & EPOLLIN)
            {
                ...
                pthread_create(&thread, NULL, worker, (void*)&fds_for_new_worker);  // 创建线程来处理事件
            }
            else
            {
